...
const shortController = {
    getAll: async(req, res) => {
        try {
            const { rows } = await postgres.query("SELECT * FROM short")
    ...
    getById: async(req, res) => {
        try {
            const { rows } = await postgres.query("SELECT * FROM short WHERE id = $1", [req.params.id])
    ...
    create: async(req, res) => {
        try {
            ...
            const { row } = await postgres.query(sql, [youtube_id, title, video_url, thumbnail_url, views, channel_id])
    ...
    updateById: async(req, res) => {
        try {
            ...
            const { rows } = await postgres.query(sql, [youtube_id, title, video_url, thumbnail_url, views, channel_id, req.params.id])
