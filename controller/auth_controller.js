...
const { authenticateToken, generateAccessToken, generateRefreshToken } = require('../token/auth_token')
const saltRounds = 10
...
const authController = {
    ...
    signup: async(req, res) => {
        try {
            ...
            if (!email || !password || !username || !name) return res.status(400).json({msg: "Please fill in all the fields"})
            ...
            let { rows } = await postgres.query(query, [username, email, hashedPassword])
            const account = rows[0]
            // insert into USER_INFO table
            const handle = '@' + username
            const defaultAvatar = 'https://cdn4.iconfinder.com/data/icons/avatars-xmas-giveaway/128/batman_hero_avatar_comics-512.png'
            const defaultThumbnail = 'https://cdn.pixabay.com/photo/2023/02/03/05/11/youtube-background-7764170_1280.jpg'
            const query2 = "INSERT INTO user_info (id, handle, name, avatar_url, thumbnail_url) VALUES ($1, $2, $3, $4, $5) RETURNING *"
            rows = await postgres.query(query2, [account.id, handle, name, defaultAvatar, defaultThumbnail])

            res.json({msg: "Signup successful", data: rows[0]})
        } catch (error) {
