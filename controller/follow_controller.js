...
const follow_controller = {
    incrementFollow: async(req, res) => {
        try {
            ...
            if(follower_id == following_id) return res.json({msg: "Can not follow yourself"})
            const exist = await postgres.query("SELECT * FROM follower WHERE host_id = $1 AND follower_id = $2",[following_id, follower_id])
            if (exist.rows.length > 0) return res.json({msg: "Can not follow anymore"})
    ...
    decrementFollow: async(req, res) => {
        try {
            const { follower_id, following_id } = req.body
            if (!follower_id || !following_id) return res.status(400).json({msg: "Please fill in all the fields"})
            if(follower_id == following_id) return res.json({msg: "Can not unfollow yourself"})
            const exist = await postgres.query("SELECT * FROM follower WHERE host_id = $1 AND follower_id = $2",[following_id, follower_id])
            if (exist.rows.length == 0) return res.json({msg: "Can not unfollow"})

            const updateFollowerQuery = "DELETE FROM follower WHERE host_id = $1 AND follower_id = $2"
            const updateFollowingQuery = "DELETE FROM following WHERE host_id = $1 AND following_id = $2"
            const updateFollowerUserInfoQuery = "UPDATE user_info SET following = following - 1 WHERE id = $1"
            const updateFollowingUserInfoQuery = "UPDATE user_info SET follower = follower - 1 WHERE id = $1"
            ...
            res.json({
                msg: "Unfollow is updated successfully",
            })
        } catch(err) {
            console.log(err)
            res.status(400).json({msg: err.msg})
